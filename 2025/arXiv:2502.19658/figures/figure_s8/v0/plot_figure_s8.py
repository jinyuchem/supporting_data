...
if __name__ == "__main__":
    ...
    if lineshape == True:
        ...
        # fraction
        print("Fraction of FC is %.5f" % (sum(abs(fc_lsp)) / sum(all_lsp)))
        print("Fraction of FCHT is %.5f" % (sum(abs(fcht_lsp)) / sum(all_lsp)))
        print("Fraction of HT is %.5f" % (sum(abs(ht_lsp)) / sum(all_lsp)))

        print("Summ of FC is %.5f" % (sum(fc_lsp) / sum(all_lsp)))
        print("Summ of FCHT is %.5f" % (sum(fcht_lsp) / sum(all_lsp)))
        print("Summ of HT is %.5f" % (sum(ht_lsp) / sum(all_lsp)))
        ...
        # bars
        fc_bars = np.zeros((order_a * order_b, order_a * order_b))
        fcht_bars = np.zeros((order_a * order_b, order_a * order_b))
        ht_bars = np.zeros((order_a * order_b, order_a * order_b))
        for ind_a in range(order_a):
            for ind_b in range(order_b):
                fc_bars[0, ind_a * order_a + ind_b] = energy_v[ind_a, ind_b]
                fcht_bars[0, ind_a * order_a + ind_b] = energy_v[ind_a, ind_b]
                ht_bars[0, ind_a * order_a + ind_b] = energy_v[ind_a, ind_b]
                fc_bars[1, ind_a * order_a + ind_b] = (
                    mu**2 * fc_int_v[ind_a, ind_b] ** 2 * BZ(ind_a * freq_a, Temp)
                )
                fcht_bars[1, ind_a * order_a + ind_b] = (
                    2
                    * mu
                    * DmuDq
                    * fc_int_v[ind_a, ind_b]
                    * fc_int_q[ind_a, ind_b]
                    * BZ(ind_a * freq_a, Temp)
                )
                ht_bars[1, ind_a * order_a + ind_b] = (
                    DmuDq**2 * fc_int_q[ind_a, ind_b] ** 2 * BZ(ind_a * freq_a, Temp)
                )
        ...
        ax.set_xlabel("$\Delta$ (eV)")
        ax.set_ylabel("$F(\Delta)$ (eV$^{-1}$)")
