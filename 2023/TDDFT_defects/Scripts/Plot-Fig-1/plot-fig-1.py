...
def read_results(fileName):
    ...
    for key in list(raw_.keys()):
        ...
        allhpsitime.append(raw_[key]["hpsi_time"])
    ...
    return allnace, alleigs, allitertime, allhpsitime, allforces
...
fname = 'results.json'
allnace, alleigs, allitertime, allhpsitime, allforces = read_results(fname)
...
ax[0][0].plot(allnace, 1e3 * error_vee[1:], c=colors[0], marker='s',
              lw=0, markersize=5,)
# nv forces error
ax[0][1].plot(allnace, 1e3 * me[1:,3], c=colors[0], marker='s',
              lw=0, markersize=5, label='Max Abs Error')
ax[0][1].plot(allnace, 1e3 * mse[1:,3], c=colors[1], marker='o',
              lw=0, markersize=5, label='MSE')
ax[0][1].plot(allnace, 1e3 * mae[1:,3], c=colors[2], marker='^',
              lw=0, markersize=5, label='MAE')
# nv total wall time
ax[1][1].plot(allnace, 100 * np.array(allitertime)[1:]/allitertime[0], c=colors[-1], marker='s',
              lw=0, markersize=5,)

# nv k1d time
ax[1][0].plot(allnace, 100 * np.array(allhpsitime)[1:]/allhpsitime[0], c=colors[-1], marker='s',
              lw=0, markersize=5,)
...
for i in range(2):
    for j in range(2):
        ...
        if i==1: ax[i][j].set_xlabel('$N_{\mathrm{ACE}}$')
        ...
        if i==1 and j==0: ax[i][j].set_ylabel('$\mathcal{D}$ wall time (%)')
        ...
        ax[i][j].set_xticks([0, 1, 2, 3, 4, 5, 6, 7, 8])
        if i==1: ax[i][j].set_xticklabels([462, 562, 662, 762, 862, 1293, 1724, 2155, 2568],
              rotation=60)
        if i==0: ax[i][j].set_xticklabels([])

        #ax[i][j].set_ylim([3.5, 6.5])

        if i==1 and j==1: ax[i][j].set_ylim([0,100])
        if i==1 and j==0: ax[i][j].set_ylim([0,1])

fig.add_subplot(111, frameon = False)
plt.tick_params(labelcolor='none', top=False, bottom=False, left=False, right=False)
plt.subplots_adjust(wspace=0.4, hspace=0.2)

plt.savefig("Fig-1.pdf", bbox_inches='tight', dpi=300)
plt.show()
